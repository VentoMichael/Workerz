...
@section('content')
    <main class="max-w-7xl mx-auto pb-10 lg:py-12 lg:px-8" x-data="{ errors: {{ json_encode($errors->keys()) }}, focusField(input) { $refs[input].scrollIntoView();console.log(errors.length) } }"
          x-init="() => {
         $nextTick(() => {
        ...
        <div class="lg:grid lg:grid-cols-12 lg:gap-x-5">
            ...
            <section id="main_content" class="space-y-6 sm:px-6 lg:px-0 lg:col-span-9">
                <section aria-labelledby="plan-heading">
                    <form action="{{route('dashboard.plan-change.post')}}" method="POST">
                        @csrf
                        <input type="hidden" name="changePlan" value="true">
                        <div class="shadow sm:rounded-md sm:overflow-hidden">
                            <div class="bg-white py-6 px-4 space-y-6 sm:p-6">
                                <div class="flex p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50" role="alert">
                                    ...
                                    <div>
                                        <span class="font-medium">You're currently on the {{ $matchedPlan }} Plan with {{ $interval }} billing, next payment due on {{ $lastDay }} ({{$matchedPrice}}€).</span>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <fieldset>
                                        ...
                                        <div x-data="{ annualBilling: false }">
                                                ...
                                                @foreach($plans as $plan)
                                                    <label
                                                        ...
                                                        <div class="flex items-center text-sm">
                                                            <input @if ($subscription->name === $plan->name) checked
                                                                   @endif wire:model.blur="plan" type="radio"
                                                                   name="plan"
                                                                   value="{{ $plan->name }}"
                                            ...
                                            <div class="flex">
                                                ...
                                                @enderror
                                                <label
                                                    class="relative ml-auto mt-4 inline-flex mb-4 items-center cursor-pointer">
                                                    <input type="checkbox"
                                                           x-model="annualBilling" name="annualBilling"
                                                           class="sr-only peer">
                                                    <div
                                                    ...
                                                    <span
                                                        class="mr-2 text-sm font-medium text-gray-900">Annual billing </span>
