        ...
        <div class="divide-y divide-gray-200">
            <div class="px-4 py-5 sm:px-6 flex justify-between">
                ...
                @if(!$comments->isEmpty())
                    ...
                    <div class="flex items-center">
                        ...
                        <p class=" text-sm font-bold text-gray-900">{{ $totalRating }}</p>
                        <span class="w-1 h-1 mx-1.5 bg-gray-500 rounded-full"></span>
                        <p class="text-sm font-medium text-gray-900">{{ count($company->comments) }} {{ count($company->comments) > 1 ? 'reviews' : 'review'  }} </p>
                    </div>
                @endif
            ...
            @if (is_array($user->company->logoUpload))
                ...
                @endforeach
                            ...
                            <div>
                                @if (isset($userHasVoted[$comment->id]) && $userHasVoted[$comment->id])
                                ...
                                @auth
                                    <div class="flex items-center mt-3 space-x-3 divide-x divide-gray-200">
                                        <button wire:click="toggleVote({{ $comment->id }})"
                                                class="text-gray-900 border {{ $userHasVoted[$comment->id] === true ? 'bg-purple-500 text-white':'' }} hover:bg-purple-500 border-gray-300 focus:outline-none focus:ring-2 focus:ring-purple-200 font-medium rounded-lg text-xs px-2 py-1.5">
                                            Helpfull
                                        </button>
                                    </div>
                                @endauth
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div>
                                <form wire:submit.prevent="addComment">
                                    <div class="mb-2">
                                        <div x-data="{ rating: 0 }" class="flex space-x-1 mb-3 items-center"
                                             id="star-rating">
                                            <p class="@error('rating') text-red-800 @enderror">Note :</p>
                                            <div x-data="{ rating: 0 }">
                                                <fieldset class="flex space-x-1 items-center">
                                                    ...
                                                    @for($i = 1; $i <= 5; $i++)
                                                        ...
                                                        >
                                                            ...
                                                            <svg
                                                                ...
                                                                fill="currentColor"
                                                                viewBox="0 0 22 20"
                                                                x-bind:class="{ 'text-yellow-300': rating >= {{$i}} && rating !== 0 }"
                                                            >
                                                                <path
                                                                    d="M20.924 7.625a1.523 1.523 0 0 0-1.238-1.044l-5.051-.734-2.259-4.577a1.534 1.534 0 0 0-2.752 0L7.365 5.847l-5.051.734A1.535 1.535 0 0 0 1.463 9.2l3.656 3.563-.863 5.031a1.532 1.532 0 0 0 2.226 1.616L11 17.033l4.518 2.375a1.534 1.534 0 0 0 2.226-1.617l-.863-5.03L20.537 9.2a1.523 1.523 0 0 0 .387-1.575Z"
                                                                />
                                    ...
                                    <div>
                                        ...
                                        <textarea id="comment" name="comment" rows="3"
                                                  class="@error('commentText')border border-red-500 @else border-gray-300 @enderror p-2 shadow-sm block w-full focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
                                                  placeholder="Ajouter un commentaire"
                                                  wire:model="commentText"></textarea>
                                        @error('commentText')
                                        <p class="text-red-500 mt-1">{{ $message }}</p>
                                        @enderror
                                    </div>
